...
#define max_servers action_count - 1   // from parameters.h 
	...
	// mat O_n(opt_steps,7);
	// O_n.col(0) = values;
	//Hypotheses<double> hypos = {&values, &belief};
	...
	cout << "UTC MC value: " << MC_utc(result.root, &belief, ims, periods) << endl;

	/* Bayes */
	cout << "Dynamic Bayes MC value: " << V_dynamic_MC(&belief, ims, periods) << endl;
